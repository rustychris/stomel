...
class OptimizeGridMixin(object):
    ...
    def cell_neighborhood_apollo(self,c):
        ...
        return unique( self.cells[cell_list,:] )
    ...
    def repave_neighborhood(self,c,neighborhood_size=3,scale_factor=None,nbr_nodes=None):
        ...
        edges_to_kill = unique( concatenate( [self.pnt2edges(n) for n in nbr_nodes] ) )
        ...
        for n in nbr_nodes:
            ...
            if self.node_on_boundary(n):
                ...
                # the boundary
                if self.node_data[n,pc.STAT] == pc.SLIDE:
                    # print "Setting node n=%d to HINT"%n
                    self.node_data[n,pc.STAT] = pc.HINT
            else:
        ...
        new_cells = arange(old_ncells,self.Ncells())
