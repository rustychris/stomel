...
import sys
sys.path.append('/home/rusty/python')

import paver,trigrid,orthomaker,field
...
class OptimizeGui(object):
    ...
    def show_bad_cells(self):
        bad = nonzero( self.og.cell_scores() <0.1 )[0]
        vc = self.p.vcenters()
        ax = axis()
        gca().texts = []
        [annotate(str(i),vc[i]) for i in bad]
        axis(ax)
    ...
    def onpress(self,event):
        if event.inaxes is not None:
            ...
            pnt = array( [event.xdata,event.ydata] )
            if event.key == 't':
...
class GridOptimizer(object):
    def __init__(self,p):
        ...
        valid = isfinite(p.points[:,0])
        xy_min = p.points[valid].min(axis=0)
        xy_max = p.points[valid].max(axis=0)

        self.scale_reductions = array( [ [xy_min[0],xy_min[1],1e6],
    ...
    def cell_scores(self,cell_ids=None,use_original_density=True):
        ...
        if cell_ids is None:
            cell_ids = arange(p.Ncells())

        valid = (p.cells[cell_ids,0]>=0)

        vc = p.vcenters()[cell_ids]

        local_scale = zeros( len(cell_ids), float64)
        ...
        #
        cell_scores = inf*ones( len(cell_ids) )
        ...
        scores = sqrt(minimum(d1,d2,d3)) / local_scale
    ...
    def cell_neighborhood_apollo(self,c):
        ...
        def dfs(cc):
            ...
            for child in self.p.cell_neighbors(cc):
                dfs(child)
        dfs(c)
        cell_list = array(list(c_set))
        
        return unique( self.p.cells[cell_list,:] )
    ...
    def repave_neighborhoods(self,score_threshold=0.1,count_threshold=5000,
        ...
        if self.use_apollo and scale_factor is not None and scale_factor < 1.0:
            ...
            to_reduce = nonzero(all_scores<score_threshold)[0]
            centers = self.p.vcenters()[to_reduce]
            orig_scales = self.original_density( centers )
            new_scales = scale_factor * orig_scales
            xyz = concatenate( [centers,new_scales[:,newaxis]], axis=1)
            self.scale_reductions = concatenate( [self.scale_reductions,xyz])
    ...
    def repave_neighborhood(self,c,neighborhood_size=3,scale_factor=None,nbr_nodes=None):
        ...
        edges_to_kill = unique( concatenate( [p.pnt2edges(n) for n in nbr_nodes] ) )
        ...
        new_cells = arange(old_ncells,p.Ncells())
