...
# import medial_axis as ma # seems obsolete
...
class CList(object):
    ...
    def append(self,d,after=None,metric=0.0):
        ...
        if not self.node_to_iters.has_key(d):
    ...
    def __contains__(self,node):
        ...
        return self.node_to_iters.has_key(node)
    ...
    def find_iter(self,d_prv,d,d_nxt):
        ...
        triple of nodes
        """
        if self.node_to_iters.has_key(d):
...
class Paving(paving_base,OptimizeGridMixin):
    ...
    # def downsample(self,factor=0.1):
    #     """ Uses the current min_density, times the given factors, to specify a lower bound
    #     and upper bound
    #     on edge lengths allowed in original rings.  re-initializes the rings afterwards.
    #     """
    # 
    #     new_rings = []
    #     new_degens = []
    # 
    #     for ri in range(len(self.original_rings)):
    #         if self.verbose>0:
    #             print "Downsampling ring",ri
    #             
    #         closed_ring = ri not in self.degenerate_rings
    #         ds_ring = downsample_linearring(self.original_rings[ri],
    #                                         factor*self.min_density,
    #                                         closed_ring=closed_ring)
    #         if len(ds_ring) < 3:
    #             print "A ring vanished during downsampling"
    #             continue
    #         
    #         if closed_ring:
    #             new_rings.append( ds_ring )
    #         else:
    #             new_degens.append( ds_ring )
    # 
    #     # re-initialize with the new rings
    #     self.initialize_rings(new_rings,new_degens)
        ...
        if best_nbr is None and shoot_ray:
            ...
            if e is not None:
                ...
                if (self.edges[e,3] == trigrid.UNMESHED) and (self.edges[e,4] == trigrid.BOUNDARY):
                    ...
                    if v in self.delaunay_neighbors(right_elt.data):
                        return new_elt.data
    ...
    def local_nodes(self,n,local_radius,steps=2):
            ...
            for stepper in tnxt,tprv:
                ...
                while 1:
                    ...
                    if straightline < local_radius or s<=steps:
                        if not local_nodes.has_key(trav.data):
        ...
        return array(local_nodes.items())
