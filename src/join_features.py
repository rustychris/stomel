...
def progress_printer(str,steps_done=None,steps_total=None):
    if steps_done is not None and steps_total is not None:
        print "%s -- %d%%"%(str,100.0*steps_done/steps_total)
    elif steps_done:
        print "%s -- %d"%(str,steps_done)
    else:
        print str
...
def merge_lines(layer=None,segments=None):
    ...
    if layer:
        ...
        next_seg = enumerate(segments).next
    ...
    while 1:
        ...
        if not endpoints.has_key( start_point ):
            endpoints[start_point] = []
        endpoints[start_point].append(fid)
        if not endpoints.has_key( end_point ):
            endpoints[end_point] = []
...
def tolerant_merge_lines(features,tolerance):
    ...
    for feat in features:
        ...
        if delta > 0.0 and delta <= tolerance:
            print "tolerant_merge: joining a loop - dist = %f"%delta
            feat[-1] = feat[0]
...
def find_exterior_ring(point_lists):
    ...
    if len(open_strings) > 1:
        print "Wanted exactly 0 or 1 open strings, got %i"%len(open_strings)
        for i in open_strings:
            print "  Open string: ",point_lists[i]
        raise Exception,"Can't figure out who is the exterior ring"

    if len(open_strings) == 1:
        print "Choosing exterior ring based on it being the only open ring"
        print "Endpoints: ",point_lists[open_strings[0]][0],point_lists[open_strings[0]][-1]
        return open_strings[0],False
    else:
        print "No open linestrings, resorting to choosing exterior ring by area"
            ...
            if prep_ext_poly and trust_prepared:
                ...
                if ext_poly.contains(int_poly):
                    ...
                    if prep_ext_poly is not None:
                        print "A feature got through the prepared query, but the real query says it's outside the exterior"
                    else:
                        print "Removing a feature that was outside the exterior ring"
        else:
            print "Removing a feature that the fast query said was outside the exterior ring"
